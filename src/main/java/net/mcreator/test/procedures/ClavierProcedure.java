...
public class ClavierProcedure {
	...
	public static void spawnVillager() {
		...
		if (world instanceof ServerLevel _level) {
			villager = new Villager(EntityType.VILLAGER, _level);
			villager.moveTo(entity.getX(), entity.getY(), entity.getZ(), world.getRandom().nextFloat() * 360F, 0);
	...
	public static void goTo(Entity ent, boolean repeat){
		...
		if(repeat){
			try{
				...
				new Thread(() -> ClavierProcedure.goTo(ent, true)).start();
			}
			catch(Exception e){}
		}
	}

	public static void goTo(BlockPos bloc, boolean repeat){
		if (ClavierProcedure.villager instanceof Mob _entity){
			_entity.getNavigation().moveTo(bloc.getX(), bloc.getY(), bloc.getZ(), 1);
			//_entity.setDeltaMovement(new Vec3(0.3, 0.3, 0.3));
		}
		if(repeat){
			try{
				TimeUnit.MILLISECONDS.sleep(300);
				new Thread(() -> ClavierProcedure.goTo(bloc, true)).start();
			}
			catch(Exception e){}
		}
	}
	...
	public static BlockPos highestBlock(){
		...
		for(Blocs bloc: ClavierProcedure.blocs){
			for(BlockPos block: bloc.blocs){
				if(block.getY() > highest_bp.getY()){
					ClavierProcedure.highest_bp = block;
				}
			}
		}
	...
	public static void activateTetris(Entity entity, BlockPos pos) {
		if (entity instanceof Player _player && !_player.level.isClientSide()){
			_player.displayClientMessage(Component.literal("wtf"), (false));
        ...
        while (!queue.isEmpty() && iterations < maxIterations) {
            ...
            for (int i = 0; i < queueSize; i++) {
                ...
                for (BlockPos neighbor : neighbors) {
                    if (!visited.contains(neighbor) && ClavierProcedure.world.getBlockState(neighbor) != Blocks.AIR.defaultBlockState()) {
                        queue.add(neighbor);
	...
	public static void displayBlocks(Blocs list_blocs){
		for(BlockPos b: list_blocs.blocs){
			replaceBloc(new BlocInfo(b.getX(), b.getY(), b.getZ(), Blocks.EXPOSED_CUT_COPPER.defaultBlockState()));
		}
	}
