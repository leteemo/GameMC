 ...
 * outside net.mcreator.tetris as this package is managed by MCreator.
 *
...
// Classe permettant gérer un ensemble de blocs tetris
...
public class Blocs {
	...
	public String type = "";
	public BlockState actualBlocType;
	...
	// Afficher tous les blocs à générer
	public void displayAll(){
		for(BlockPos bloc: blocs){
			ClavierProcedure.world.setBlock(bloc, actualBlocType, 3);
		}
	}

	public BlockState getActualBlocType(){
		return actualBlocType;
	}
	...
	// Faire une rotation des blocs par rapport au centre
	...
	public void moveDown(int moveX, int moveY){
		...
		for(BlockPos bloc: this.blocs){
			if(!(ClavierProcedure.world.getBlockState(new BlockPos(bloc.getX(), bloc.getY()+moveY, bloc.getZ())) == Blocks.AIR.defaultBlockState() || coordInBlocs(bloc.getX()+moveX, bloc.getY()+moveY, bloc.getZ()))) {
				stop = true;
				ClavierProcedure.goTo(ClavierProcedure.highestBlock());
			}
	...
	// Initialise les types des blocs minecraft dont sera composé le bloc tetris
    public void setRandomBloc() {
        Random random = new Random();
        int numero = random.nextInt(3);
		if (numero == 0){
			actualBlocType = Blocks.REDSTONE_BLOCK.defaultBlockState();
		}
		if (numero == 1){
			actualBlocType = Blocks.LIME_TERRACOTTA.defaultBlockState();
		}
		if (numero == 2){
			actualBlocType = Blocks.LAPIS_BLOCK.defaultBlockState();
		}
    }
	...
	// Ajouter un bloc minecraft au bloc tetris
